...
// it inserts the resrvations into the database
func (m *postgresDBRepo) InsertReservation(res models.Reservation) (int, error) {
	...
	stmt := `INSERT INTO reservation (first_name, last_name, email, phone, start_date, end_date,
...
// it returns true if availability exists for roomID, and false if not
...
// it returns slice of rooms, if any, for given date range
...
func (m *postgresDBRepo) Authenticate(email, testPassword string) (int, string, error) {
	...
	if err == bcrypt.ErrMismatchedHashAndPassword {
		return 0, "", errors.New("incorerct password")
	} else if err != nil {
		return 0, "", err
	}
...
func (m *postgresDBRepo) UpdateReservation(u models.Reservation) error {
	...
	query := `update reservation set first_name = $1, last_name = $2, email = $3, phone = $4, updated_at = $5
	          where id =$6`
...
func (m *postgresDBRepo) GetRestrictionForRoomByID(roomID int, start, end time.Time) ([]models.RoomRestriction, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
